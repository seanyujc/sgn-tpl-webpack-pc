declare namespace wx {
    // 通过config接口注入权限验证配置
    function config(para: ConfigPara);
    // config信息验证后会执行ready方法，所有接口调用都必须在config接口获得结果之后，config是一个客户端的异步操作，所以如果需要在页面加载时就调用相关接口，则须把相关接口放在ready函数中调用来确保正确执行。对于用户触发时才调用的接口，则可以直接调用，不需要放在ready函数中。
    function ready(fun: Function);
    // config信息验证失败会执行error函数，如签名过期导致验证失败，具体错误信息可以打开config的debug模式查看，也可以在返回的res参数中查看，对于SPA可以在这里更新签名。
    function error(fun: Function);
    /*基础接口*/
    // 判断当前客户端版本是否支持指定JS接口
    ...
    // 预览图片接口
    function previewImage(para: PreviewImagePara);
    // 上传图片接口
    function uploadImage(para: UploadImagePara);
    // 下载图片接口
    function downloadImage(para: DownloadImagePara);
    /*音频接口*/
    // 开始录音接口
    function startRecord();
    // 停止录音接口
    function stopRecord(para: StopRecordPara);
    // 监听录音自动停止接口
    function onVoiceRecordEnd(para: OnVoiceRecordEndPara);
    /*智能接口*/
    /*设备信息*/
    /*地理位置*/
    // 使用微信内置地图查看位置接口
    function openLocation(para: OpenLocationPara);
    // 获取地理位置接口
    function getLocation(para: GetLocationPara);
    /*摇一摇周边*/
    /*界面操作*/
    /*微信扫一扫*/
    // 调起微信扫一扫接口
    function scanQRCode(para: ScanQRCodePara);
    /*微信小店*/
    /*微信卡券*/
    /*微信支付*/
    // 发起一个微信支付请求
    function chooseWXPay(para: ChooseWXPayPara);
    ...
    interface GetLocationPara {
        ...
        type: string;
        success(res: GetLocationSuccessResult): any;

    }

    interface GetLocationSuccessResult extends Result {
        // 纬度，浮点数，范围为90 ~ -90
        latitude: number;
        // 经度，浮点数，范围为180 ~ -180。
        longitude: number;
